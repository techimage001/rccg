(() => {
  ...
  const API='api/';
  const state={content:null,route:'home',lessonMode:'teacher',currentLesson:null,currentBody:null,lessonFilter:'all',signupShown:false,eventMonth:null,formReturn:'home',audio:{chunks:[],index:0,utterance:null,playing:false,paused:false}};
  ...
  function bindGlobal(){
    ...
    $('#closeInstall')?.addEventListener('click',()=>$('#installModal')?.classList.remove('open'));
    $('#keepEditing')?.addEventListener('click',()=>$('#discardModal')?.classList.remove('open'));
    $('#discardForm')?.addEventListener('click',()=>{const f=$('#dynamicForm');if(f){f.reset();$$('.star-rating label').forEach(l=>l.classList.remove('chosen'));}leaveForm();});$('#drawerSignup')?.addEventListener('click',()=>{ $('#drawer')?.classList.remove('open'); openSignup(); });
  ...
  function route(name,push=true){
    ...
    if(page==='forms'){if(!String(previous||'').startsWith('form/'))state.formReturn=previous||'home';}
    else{$('#discardModal')?.classList.remove('open');}
    document.body.classList.toggle('form-open',page==='forms');
  ...
  function renderNext(){
    ...
    el.innerHTML=`<div class="card"><time>${fmtDateTime(e.start_at)}</time><h3>${esc(e.title)}</h3><p>${esc(e.description||'')}</p>${e.fellowship_note?`<p class="fellowship-note">${esc(e.fellowship_note)}</p>`:''}<button class="btn btn-primary" type="button" data-route="whats-on">See What's On →</button></div>`;
  }
  ...
  function renderEvents(){
    ...
    el.innerHTML=events.map(e=>{const live=(e.facebook_live||e.youtube_live)?`<div class="sunday-live"><strong>Join us live on Sunday</strong>${e.facebook_live&&st.facebook?`<a class="btn btn-facebook" data-external href="${esc(st.facebook)}">Facebook Live ↗</a>`:''}${e.youtube_live&&st.youtube?`<a class="btn btn-youtube" data-external href="${esc(st.youtube)}">YouTube Live ↗</a>`:''}</div>`:'';return `<article class="card"><div class="eyebrow" style="color:var(--blue)">${fmtDateTime(e.start_at)}</div><h3>${esc(e.title)}</h3><p>${esc(e.description||'')}</p>${e.fellowship_note?`<p class="fellowship-note">${esc(e.fellowship_note)}</p>`:''}${e.location?`<p>📍 ${esc(e.location)}</p>`:''}${e.join_url?`<a data-external class="btn btn-primary" href="${esc(e.join_url)}">Join / Open ↗</a>`:''}${live}</article>`;}).join('')||'<div class="empty">No church activities are scheduled for this month.</div>';
  }
  ...
  function renderServiceTimes(){
    ...
    el.innerHTML=`<div class="timezone-note">🕒 Service times use ${esc(tz)} and automatically follow local clock changes.</div>`+rows.map(r=>`<div class="card"><strong>${esc(r.day_label)} · ${esc(r.time_label)}</strong><p>${esc(r.service_name)}</p>${r.fellowship_note?`<p class="fellowship-note">${esc(r.fellowship_note)}</p>`:''}${r.note?`<small>${esc(r.note)}</small>`:''}</div>`).join('');
  }
  function renderChurchInfo(){
    const el=$('#churchInfo');if(!el)return;const cards=state.content?.contact_cards||[];
    el.innerHTML=`<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(220px,1fr))">${cards.map(c=>{let href='';if(c.action_type==='tel')href='tel:'+String(c.value||'');else if(c.action_type==='email')href='mailto:'+String(c.value||'');else if(c.action_type==='whatsapp')href='https://wa.me/'+String(c.value||'').replace(/[^0-9]/g,'');else if(c.action_type==='external')href=String(c.value||'');const sameWindow=(c.action_type==='tel'||c.action_type==='email');const button=href&&c.button_label?`<a class="btn ${esc(c.button_class||'')}" ${sameWindow?'':'data-external'} href="${esc(href)}">${esc(c.button_label)}</a>`:'';const shown=c.display_value?(c.action_type==='email'?`<p><a class="plain-link" href="${esc(href)}">${esc(c.display_value)}</a></p>`:`<p>${esc(c.display_value)}</p>`):'';return `<div class="card"><h3>${esc(c.icon||'')} ${esc(c.title)}</h3><p>${esc(c.subtitle||'')}</p>${shown}${button}</div>`;}).join('')}</div>`;
  }
  ...
  function renderForm(type){
    ...
    $('#formClose')?.addEventListener('click',closeCurrentForm);
  }
  function formIsDirty(form){
    if(!form)return false;
    const ignore=new Set(['website','started','form_type']);
    for(const el of form.elements){
      if(!el.name||ignore.has(el.name))continue;
      if(el.type==='submit'||el.type==='button'||el.type==='hidden')continue;
      if(el.type==='radio'||el.type==='checkbox'){if(el.checked)return true;continue}
      if(String(el.value||'').trim()!=='')return true;
    }
    return false;
  }
  function leaveForm(){const back=state.formReturn||'home';state.formReturn='home';$('#discardModal')?.classList.remove('open');route(back);}
  function closeCurrentForm(){if(!formIsDirty($('#dynamicForm'))){leaveForm();return}$('#discardModal')?.classList.add('open');}
